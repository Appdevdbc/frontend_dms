  ...
  <div class="q-pa-md">
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Revisi Pemusnahan Arsip</div>
    ...
    <q-dialog v-model="showConfirmDialog" persistent>
      <q-card class="tw-rounded-2xl tw-overflow-hidden">
        ...
        <q-card-section class="tw-p-6">
          <p class="tw-text-slate-700">Apakah anda yakin untuk mengirim ulang pemusnahan arsip ini?</p>
        </q-card-section>
...
const getTicketData = async () => {
  try {
    ...
    tmpForm.target_pemusnahan = header.tr_target_pemusnahan;
    tmpForm.tgl_pemusnahan = header.tr_tgl_pemusnahan?.split('T')[0];
    tmpForm.kategori_keamanan = header.tr_kategori_keamanan;
    tmpForm.kategori_dokumen = header.tr_kategori_dokumen;
    tmpForm.lokasi_arsip = header.tr_lokasi_arsip;
    tmpForm.keterangan_pemusnahan = header.tr_keterangan_pemusnahan;
    tmpForm.pinjam_prioritas_approve = header.tr_prioritas_approval;
    tmpForm.arsiparis = header.tr_arsiparis_id;

    // Map details to documents
...
const savePemusnahan = async () => {
  try {
    ...
    const res = await axios.put(`${import.meta.env.VITE_API}dms/pemusnahan/${route.params.id}/revise`, {
      ...tmpForm,
      creator: empid()
    }, {
      skipErrorInterceptor: true
    ...
    loading.value = false;
    success('Revisi pemusnahan berhasil dikirim');
    router.push('/dms/pemusnahan');
  } catch (error) {
    loading.value = false;
...
onMounted(async () => {
  await getTicketData();
  await getLokasiArsip();
  await getArsiparis();
  await getArsiparisAtasan();
  getKategoriKeamanan();
  getKategoriDokumen();
