         ...
         <div class="tw-relative tw-z-10">
           <div class="tw-flex tw-items-center tw-gap-3 tw-mb-2">
             <div class="tw-bg-white/20 tw-p-2 tw-rounded-lg tw-backdrop-blur-sm">
               <q-icon name="summarize" size="sm" class="tw-text-white" />
             </div>
             <div class="text-h6 tw-font-bold tw-text-white tw-drop-shadow-lg">Laporan Rekapitulasi Arsip Bulanan</div>
           </div>
           <div class="tw-flex tw-items-center tw-gap-1 tw-text-white/90 tw-text-sm">
              ...
              :to="{ path: '/collection/dms-rekapitulasi' }">
               Laporan Rekapitulasi Arsip Bulanan
              </router-link>
          </div>
         </div>
      </q-card-section>
...
const filterMenu = async (val) => {
      if (val === '') {
            ...
            listData.value = listData1.value.filter(option => {
                return option.menu_name.toLowerCase().includes(neddle)
            }) 
        }
}
