    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-5`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div class="text-h6 tw-text-white tw-font-bold">Approval Perpanjangan — Atasan User</div>
        </div>
      </q-card-section>

      <q-card-section class="tw-p-6" v-if="tiketData">
        <div class="tw-grid tw-grid-cols-2 tw-gap-4 tw-mb-6 tw-p-4 tw-bg-orange-50 tw-rounded-xl">
          ...
          <div><div class="tw-text-xs tw-text-gray-500">Catatan User</div><div class="tw-font-bold">{{ tiketData.panjang1_ket_user || '-' }}</div></div>
        ...
        <q-input v-model="catatan" outlined type="textarea" label="Catatan Atasan" rows="3" class="tw-rounded-lg">
          <template v-slot:prepend><q-icon name="notes" color="orange-6" /></template>
        </q-input>
      </q-card-section>
...
const buildPayload = () => ({ creator: empid(), no_tiket: tiketData.value.pinjam_no_tiket, id: tiketData.value.id, panjang1_ket_atasan_user: catatan.value });

const approve = async () => {
  saving.value = true;
  try {
    await axios.post(`/dms/pengembalian-perpanjangan/perpanjangan/atasan-user/approve`, buildPayload());
...
const tolak = async () => {
  ...
  saving.value = true;
  try {
    await axios.post(`/dms/pengembalian-perpanjangan/perpanjangan/atasan-user/tolak`, buildPayload());
