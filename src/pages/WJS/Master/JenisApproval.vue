<template>
  <div class="q-pa-md">
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Master Jenis Approval</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px"/>
              <q-icon name="chevron_right" size="14px"/>
              <router-link :to="{ path: '/dms/master-jenis-approval' }" class="tw-text-white hover:tw-text-blue-200 tw-transition-colors tw-no-underline">
                Master Jenis Approval
              </router-link>
            </div>
          </div>
        </div>
      ...
      <q-card-section class="tw-p-6">
        ...
        >
          ...
          <template v-slot:body-cell-app_jns_status="props">
            <q-td :props="props" class="tw-py-4">
              <q-badge 
                :color="props.row.app_jns_status == 1 ? 'green-6' : 'red-6'" 
                :label="props.row.app_jns_status == 1 ? 'Active' : 'Inactive'"
                class="tw-px-3 tw-py-1 tw-font-semibold tw-rounded-full"
              >
                <q-icon 
                  :name="props.row.app_jns_status == 1 ? 'check_circle' : 'cancel'" 
                  size="14px" 
                  class="tw-mr-1"
                />
              </q-badge>
    ...
    <q-dialog v-model="dialogForm" transition-show="slide-up" transition-hide="slide-down">
      <q-card class="tw-w-full tw-max-w-lg tw-rounded-2xl tw-shadow-2xl" icon="approval">
...
const columns = [
  ...
  },
  {
    ...
    label: "Status",
    align: "left",
    field: "app_jns_status",
    sortable: true,
  },
];
const $q = useQuasar();
const listJenisApproval = ref([]);
...
const schema = yup.object({
  jenis:yup.string().required("Jenis Approval wajib diisi").nullable(),
});

const getJenisApproval = async () => {
  try {
    loading.value = true;
    ...
    // listJenisApproval.value = res.data.data;
    if (typeof res.data.data === "undefined") {
      listJenisApproval.value = res.data;
    } else {
      listJenisApproval.value = res.data.data;
    }
...
const editJenis = async (value) => {
  try {
    reset();
    updateForm.value = true;
    tmpForm.id = value.app_jns_id;
    tmpForm.jenis = value.app_jns_desc;
    tmpForm.status = value.app_jns_status;
    dialogForm.value = true;
  } catch (err) {
    console.log(err)
...
const validateApp = async () => {
  schema.validate({ jenis: tmpForm.jenis }, { abortEarly: false })
  .then(() => {
      saveJenisApproval();
    })
    .catch(err => {
...
const saveJenisApproval = async () => {
  if (updateForm.value) {
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveJenisApproval`, {
        ...tmpForm,
        update: true
      }, {
        skipErrorInterceptor: true
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveJenisApproval`, tmpForm, {
        skipErrorInterceptor: true
      });
      dialogForm.value = false;
      reset();
...
const getPageAkses = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}pageakses`, {
      params: {
        role:empid(),
        page:'/dms/master-jenis-approval',
        domain:domain(),
      },
      skipErrorInterceptor: true
    });
