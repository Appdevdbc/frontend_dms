    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Master Worklocation</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px"/>
              <q-icon name="chevron_right" size="14px"/>
              <router-link :to="{ path: '/dms/master-work-location' }" class="tw-text-white hover:tw-text-blue-200 tw-transition-colors tw-no-underline">
                Master Worklocation
              </router-link>
            </div>
          </div>
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12">
              <q-select
                ...
                :options="listWork"
                option-value="work_location_code"
                @filter="filterWork"
                option-label="work_location_name"
                label-slot
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
                ...
                <template v-slot:label>
                  <span class="tw-font-semibold tw-text-slate-700">Lokasi Kerja</span>
            ...
            <div class="col-12">
              ...
              >
                ...
                <template v-slot:label>
                  <span class="tw-font-semibold tw-text-slate-700">Kode Lokasi Kerja</span>
                  <span class="tw-text-red-500 tw-font-bold">*</span>
                </template>
              </q-input>
            </div>
...
const schema = yup.object({
  work:yup.string().required("Lokasi Kerja wajib diisi").nullable(),
  kode:yup.string().required("Kode Lokasi Kerja wajib diisi").nullable(),
});
...
const getWorkLocation = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}dms/listWorkLocation`, {
      params: pagination.value,
      skipErrorInterceptor: true
    });
    // listLocation.value = res.data.data;
    if (typeof res.data.data === "undefined") {
      listLocation.value = res.data;
    } else {
      listLocation.value = res.data.data;
    }
...
const editWork = async (value) => {
  try {
    reset();
    updateForm.value = true;
    tmpForm.kode = value.work_code;
    tmpForm.work = value.work_id;
    tmpWork.code = tmpForm.work
    dialogForm.value = true;
    await getListWorkLocation()
  } catch (err) {
    console.log(err)
...
const deleteWork = async (value) => {
  $q.dialog({
    title: "Konfirmasi",
    message: `Apakah anda ingin menghapus data <span class="text-bold">${value.work_desc}</span>?`,
    html: true,
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/deleteWork`, {
          id:value.work_id,
          creator:empid()
          }, {
          skipErrorInterceptor: true
...
const saveWork = async () => {
  if (updateForm.value) {
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveWorkLocation`, {
        ...tmpForm,
        update: true
      }, {
        skipErrorInterceptor: true
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveWorkLocation`, tmpForm, {
        skipErrorInterceptor: true
      });
      dialogForm.value = false;
      reset();
...
const getListWorkLocation = async () => {
  try {
    const res = await axios.get(`${import.meta.env.VITE_API}dms/listVwWorkLocation`, {
      params: {
        ...tmpWork,
      },
      skipErrorInterceptor: true
    });
    listWork.value = res.data.map(item => ({
      ...item,
      plain_id: item.work_id
    }));
  } catch (error) {
    console.error('getListWorkLocation error:', error);
  } 
};

const filterWork = async (val, update) => {
      if (val === '') {
            update(() => {
                tmpWork.needle = null;
                tmpWork.code = null;
        ...
        update(() => {
            tmpWork.needle = val.toLowerCase();
            getListWorkLocation();
        })
}
