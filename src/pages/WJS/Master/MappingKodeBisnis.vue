    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Mapping Kode Bisnis Arsip Unit</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px"/>
              <q-icon name="chevron_right" size="14px"/>
              <router-link :to="{ path: '/dms/mapping-bisnis-unit' }" class="tw-text-white hover:tw-text-blue-200 tw-transition-colors tw-no-underline">
                Mapping Kode Bisnis Arsip Unit
              </router-link>
            </div>
          </div>
        </div>
      ...
      <q-card-section class="tw-p-6">
        ...
        >
          ...
          <template v-slot:body-cell-map_kd_bu_status="props">
            <q-td :props="props" class="tw-py-4">
              <q-badge 
                :color="props.row.map_kd_bu_status == 1 ? 'green-6' : 'red-6'" 
                :label="props.row.map_kd_bu_status == 1 ? 'Active' : 'Inactive'"
                class="tw-px-3 tw-py-1 tw-font-semibold tw-rounded-full"
              >
                <q-icon 
                  :name="props.row.map_kd_bu_status == 1 ? 'check_circle' : 'cancel'" 
                  size="14px" 
                  class="tw-mr-1"
                />
              </q-badge>
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12">
              ...
              >
                ...
                <template v-slot:label>
                  <span class="tw-font-semibold tw-text-slate-700">Kode BU</span>
                  <span class="tw-text-red-500 tw-font-bold">*</span>
                </template>
              </q-input>
            </div>
            <div class="col-12">
              ...
              >
                ...
                <template v-slot:label>
                  <span class="tw-font-semibold tw-text-slate-700">Kode Singkat BU</span>
                  <span class="tw-text-red-500 tw-font-bold">*</span>
                </template>
              </q-input>
            </div>
...
const columns = [
  ...
  },
  {
    name: "map_bu_singkat",
    required: true,
    label: "Kode Singkat BU",
    align: "left",
    field: "map_bu_singkat",
    sortable: true,
  },
  {
    name: "map_kd_bu_status",
    required: true,
    label: "Status",
    align: "left",
    field: "map_kd_bu_status",
  },
];
const $q = useQuasar();
const listMappingUnit = ref([]);
const listUser = ref([]);
...
const schema = yup.object({
  ...
  name: yup.string().required("Kode BU wajib diisi").nullable(),
  kode: yup.string().required("Kode Singkat BU wajib diisi").nullable(),
});

const getMappingBisnis = async () => {
  try {
    loading.value = true;
    ...
    // listMappingUnit.value = res.data.data;
    if (typeof res.data.data === "undefined") {
      listMappingUnit.value = res.data;
    } else {
      listMappingUnit.value = res.data.data;
    }
...
const editMapping = async (value) => {
  try {
    console.log(value)
    reset();
    updateForm.value = true;
    await getDomains();
    tmpForm.id = value.map_kd_bu_id;
    tmpForm.domain = value.map_mstr_bu_id;
    tmpForm.name = value.map_desc_kd_bu;
    tmpForm.kode = value.map_bu_singkat;
    tmpForm.status = value.map_kd_bu_status;
    dialogForm.value = true;
  } catch (err) {
    console.log(err)
...
const saveMapping = async () => {
  if (updateForm.value) {
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveMappingBisnisUnit`, {
        ...tmpForm,
        update: true
      }, {
        skipErrorInterceptor: true
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API}dms/saveMappingBisnisUnit`, tmpForm, {
        skipErrorInterceptor: true
      });
      dialogForm.value = false;
      reset();
...
const downloadExcel = async () => {
  try {
    ...
    link.href = url;
    link.setAttribute('download', 'Mapping_Kode_Bisnis_Unit.xlsx');
