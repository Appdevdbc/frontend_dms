    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      ...
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Service Level SPK Repair (YTD)</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px" />
              <q-icon name="chevron_right" size="14px" />
              <span>Report</span>
              <q-icon name="chevron_right" size="14px" />
              <span>SLA Repair YTD</span>
            </div>
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section class="tw-bg-white tw-flex tw-flex-col tw-gap-6">
        ...
        <template v-if="chartsReady && !loading">
            ...
            <q-card-section :class="`side-${domain()}-1 tw-py-3 tw-px-5`">
              <div class="tw-flex tw-items-center tw-justify-between">
                <div class="tw-flex tw-items-center tw-gap-2 tw-text-white tw-font-semibold">
                  ...
                  Service Level SPK Repair (YTD) — {{ selectedMonth }}
                </div>
          ...
          <!-- Target VS Pencapaian + SPK Outstanding -->
          <div class="row q-col-gutter-md">
            <div class="col-12 col-md-6">
              <q-card flat bordered class="tw-rounded-xl tw-overflow-hidden">
...
const JENIS = 'repair'; // only difference from SlaGeneralYTD
...
function buildYTDChart(data, year) {
  ytdChartOptions.value = {
    ...baseHC(),
    chart: { zoomType: 'xy' },
    title: { text: 'Service Level SPK Repair', style: { fontSize: '14px' } },
    xAxis: [{ categories: data.map(d => d.month), crosshair: true }],
    yAxis: [{ title: { text: 'Service Level (%)' }, labels: { format: '{value} %' } }],
    tooltip: { shared: true },
    ...
    series: [
      {
        ...
        data: data.map(d => parseFloat(d.sla) || 0),
        tooltip: { valueSuffix: '%' },
        cursor: 'pointer',
        point: { events: { click: function () { loadDetail(this.category, year); } } },
      },
      {
        name: 'Target Service Level', type: 'spline', color: '#ec5252',
...
function buildTargetChart(target, tercapai) {
  targetChartOptions.value = columnChart('Target VS Pencapaian', ['Target', 'Pencapaian'], [
...
function buildOutstandingChart(open, close, outstanding, year) {
  outstandingChartOptions.value = columnChart(`Tipe SPK ${year}`, ['Open', 'Close', 'Outstanding'], [
...
async function loadLevel1(bulan, year) {
  try {
    const [hariRes, jamRes] = await Promise.all([
      axios.get(`${import.meta.env.VITE_API}getSlaGeneralYTDHari`, { params: { bulan, tahun: year, jenis: JENIS } }),
      axios.get(`${import.meta.env.VITE_API}getSlaGeneralYTDJam`,  { params: { bulan, tahun: year, jenis: JENIS } }),
    ]);
    ...
    hariChartOptions.value = {
      ...
      xAxis: [{ categories: hariRes.data.map(d => d.id_spk), crosshair: true }],
      yAxis: [{ title: { text: 'Total Hari' }, labels: { format: '{value} Hari' } }],
      tooltip: { shared: true },
      legend: { layout: 'horizontal', align: 'center', verticalAlign: 'bottom' },
      series: [{
        name: 'SPK', type: 'column', color: '#33bad0',
        data: hariRes.data.map(d => parseInt(d.total_hari) || 0),
        tooltip: { valueSuffix: ' Hari' },
        cursor: 'pointer',
        point: { events: { click: function () { loadLevel2(this.category); } } },
      }],
    };
    ...
    jamChartOptions.value = {
      ...
      xAxis: [{ categories: jamRes.data.map(d => d.id_spk), crosshair: true }],
      yAxis: [{ title: { text: 'Total Jam' }, labels: { format: '{value} Jam' } }],
      tooltip: { shared: true },
      legend: { layout: 'horizontal', align: 'center', verticalAlign: 'bottom' },
      series: [{
        name: 'SPK', type: 'column', color: '#33bad0',
        data: jamRes.data.map(d => parseInt(d.total_jam) || 0),
...
const searchData = async () => {
  try {
    ...
    const [ytdRes, summaryRes] = await Promise.all([
      axios.get(`${import.meta.env.VITE_API}getSlaGeneralYTD`,        { params: { year, jenis: JENIS } }),
      axios.get(`${import.meta.env.VITE_API}getSlaGeneralYTDSummary`, { params: { year, jenis: JENIS } }),
    ]);

    const s = summaryRes.data;
    buildYTDChart(ytdRes.data, year);
    buildTargetChart(s.target, s.tercapai);
    buildOutstandingChart(s.slaOpen, s.slaClose, s.slaOutstanding, year);
