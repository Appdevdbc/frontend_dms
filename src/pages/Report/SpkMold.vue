    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      ...
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Laporan SPK Mold</div>
...
const loadDepts = async () => {
  try {
    const res = await axios.get(`${import.meta.env.VITE_API}getDeptListMold`);
    deptOptions.value = [
...
function buildMainChart(data, year, month) {
  mainChartOptions.value = {
    ...baseHC(),
    chart: { zoomType: 'xy' },
    title: { text: 'Total SPK Mold', style: { fontSize: '14px' } },
...
const searchData = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}getSpkMold`, {
      params: {
