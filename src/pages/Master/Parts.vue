    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          ...
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Parts Master</div>
      ...
      <q-card class="tw-w-full tw-max-w-2xl tw-rounded-2xl tw-shadow-2xl">
        ...
        <q-card-section class="tw-p-6">
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-input
                v-model="form.code_part"
                outlined
                label="Part Code"
                :rules="[val => !!val || 'Part code is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
            ...
            <div class="col-12">
              <q-input
                ...
                outlined
                label="Part Name"
                :rules="[val => !!val || 'Part name is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
...
const getData = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}listParts`, {
...
const openEditDialog = async (row) => {
  isEdit.value = true;
  form.id_part = row.id_part;
  form.nama_part = row.nama_part;
  form.code_part = row.code_part;
  dialogForm.value = true;
};
...
const saveData = async () => {
  ...
  try {
    ...
    const payload = {
      id_part: form.id_part,
      nama_part: form.nama_part,
      code_part: form.code_part,
      creator: empid(),
    };
    
    await axios.post(`${import.meta.env.VITE_API}savePart`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'Part updated successfully' : 'Part added successfully');
    onRequest({
    ...
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save part');
  }
};
...
const deleteData = async (row) => {
  $q.dialog({
    ...
    message: `Are you sure you want to delete part <strong>${row.nama_part}</strong>?`,
    html: true,
    ...
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deletePart`, {
        id_part: row.id_part_encrypted,
        creator: empid(),
      });
      ...
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete part');
    }
  });
};
