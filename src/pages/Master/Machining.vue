    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="precision_manufacturing" size="28px" class="tw-text-white" />
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Machining Process</div>
      ...
      <q-card class="tw-w-full tw-max-w-2xl tw-rounded-2xl tw-shadow-2xl">
        <q-card-section :class="`bg-${domain()}`">
          <div class="text-h5 tw-text-white tw-font-bold tw-flex tw-items-center tw-gap-3">
            ...
            {{ isEdit ? 'Edit' : 'Add' }} Machining Process
          </div>
        </q-card-section>
        <q-separator/>
          ...
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-input
                ...
                outlined
                label="Process Name"
                :rules="[val => !!val || 'Process name is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
...
const getData = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}listMachiningProses`, {
      params: pagination.value,
    });
    
    if (typeof res.data.data === "undefined") {
      listMachining.value = res.data;
    } else {
      listMachining.value = res.data.data;
...
const saveData = async () => {
  ...
  try {
    ...
    const payload = {
      id_proses: form.id_proses,
      nama: form.nama,
      creator: empid(),
    };
    
    await axios.post(`${import.meta.env.VITE_API}saveMachiningProses`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'Process updated successfully' : 'Process added successfully');
    onRequest({
    ...
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save process');
  }
};
...
const deleteData = async (row) => {
  $q.dialog({
    ...
    message: `Are you sure you want to delete process <strong>${row.nama}</strong>?`,
    html: true,
    ...
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deleteMachiningProses`, {
        id_proses: row.id_proses_encrypted,
        creator: empid(),
      });
      ...
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete process');
    }
  });
};
