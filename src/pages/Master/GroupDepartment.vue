<template>
  <div class="q-pa-md">
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="group_work" size="28px" class="tw-text-white" />
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Group Department</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px"/>
              <q-icon name="chevron_right" size="14px"/>
              <span>Master</span>
              <q-icon name="chevron_right" size="14px"/>
              <span>Group Department</span>
            </div>
          </div>
        </div>
      </q-card-section>
      <q-separator />
      <q-card-section class="tw-bg-white">
        <q-table
          :rows="listGroupDepartments"
          :columns="columns"
          row-key="id_group"
          v-model:pagination="pagination"
          :rows-per-page-options="[]"
          :loading="loading"
          :filter="pagination.filter"
          @request="onRequest"
          binary-state-sort
          flat
          class="tw-shadow-md tw-rounded-xl tw-overflow-hidden"
        >
          <template v-slot:header="props">
            <q-tr :props="props">
                <q-th
                    v-for="col in props.cols"
                    :key="col.name"
                    :props="props"
                    :class="[
                      `bg-${domain()} tw-text-white tw-font-bold tw-text-sm tw-uppercase tw-tracking-wide tw-py-4`,
                      col.name === 'aksi' ? 'sticky-column-left-header' : ''
                    ]"
                >
                        {{ col.label }}
                </q-th>
            </q-tr>
          </template>
          
          <template v-slot:top-left>
            <div class="tw-flex tw-items-center tw-gap-2 tw-bg-white tw-px-4 tw-py-2 tw-rounded-lg tw-shadow-sm">
              <q-icon name="view_headline" color="blue-6" size="20px">
                <q-tooltip class="tw-bg-slate-800 tw-text-xs">Rows per page</q-tooltip>
              </q-icon>
              <q-select
                borderless
                dense
                v-model="pagination.rowsPerPage"
                :options="[10,25,50,100]"
                @update:modelValue="updateTable"
                class="tw-min-w-[80px]"
              />
            </div>
          </template>
          
          <template v-slot:top-right>
            <div class="tw-flex tw-gap-3 tw-items-center">
              <q-btn
                unelevated
                :color="`${domain()}`"
                label="Add Group"
                icon="add"
                @click="openAddDialog"
                class="tw-font-semibold tw-px-4 tw-rounded-lg hover:tw-brightness-110 tw-transition-all"
              />
              <q-input
                outlined
                dense
                debounce="300"
                v-model="pagination.filter"
                placeholder="Search..."
                class="tw-bg-white tw-rounded-lg tw-shadow-sm tw-min-w-[300px]"
              >
                <template v-slot:prepend>
                  <q-icon name="search" :color="`${domain()}`" />
                </template>
              </q-input>
            </div>
          </template>
          
          <template v-slot:body-cell="props">
            <q-td :props="props" class="tw-py-4 tw-text-slate-700">
              {{ props.value }}
            </q-td>
          </template>
          
          <template v-slot:body-cell-dept="props">
            <q-td :props="props" class="tw-py-4 tw-text-slate-700">
              <div v-html="props.value"></div>
            </q-td>
          </template>
          
          <template v-slot:body-cell-aksi="props">
            <q-td :props="props" class="tw-py-3 sticky-column-left">
              <div class="tw-flex tw-gap-2">
                <q-btn
                  round
                  dense
                  color="blue-6"
                  icon="edit"
                  size="sm"
                  @click="openEditDialog(props.row)"
                  class="tw-shadow-md hover:tw-shadow-lg hover:tw-scale-110 tw-transition-all"
                >
                  <q-tooltip class="tw-bg-slate-800 tw-text-xs">Edit</q-tooltip>
                </q-btn>
                
                <q-btn
                  round
                  dense
                  color="red-6"
                  icon="delete"
                  size="sm"
                  @click="deleteData(props.row)"
                  class="tw-shadow-md hover:tw-shadow-lg hover:tw-scale-110 tw-transition-all"
                >
                  <q-tooltip class="tw-bg-slate-800 tw-text-xs">Delete</q-tooltip>
                </q-btn>
              </div>
            </q-td>
          </template>
        </q-table>
      </q-card-section>
    </q-card>

    <!-- Add/Edit Dialog -->
    <q-dialog v-model="dialogForm" transition-show="slide-up" transition-hide="slide-down" persistent>
      <q-card class="tw-w-full tw-max-w-4xl tw-rounded-2xl tw-shadow-2xl">
        <q-card-section :class="`bg-${domain()}`">
          <div class="text-h5 tw-text-white tw-font-bold tw-flex tw-items-center tw-gap-3">
            <q-icon name="group_work" size="32px"/>
            {{ isEdit ? 'Edit' : 'Add' }} Group Department
          </div>
        </q-card-section>
        <q-separator/>
        <q-card-section class="tw-p-6">
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-input
                v-model="form.nama"
                outlined
                label="Group Name"
                :rules="[val => !!val || 'Group name is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
                  <q-icon name="label" color="blue-6"/>
                </template>
              </q-input>
            </div>
            
            <div class="col-6">
              <div class="tw-font-semibold tw-text-slate-700 tw-mb-2">Available Departments</div>
              <q-list bordered class="tw-rounded-lg tw-max-h-[400px] tw-overflow-auto">
                <q-item 
                  v-for="dept in availableDepts" 
                  :key="dept.id_dept"
                  clickable
                  @click="addDepartment(dept)"
                  class="hover:tw-bg-blue-50"
                >
                  <q-item-section avatar>
                    <q-icon name="add_circle" color="green-6"/>
                  </q-item-section>
                  <q-item-section>
                    <q-item-label>{{ dept.nama }}</q-item-label>
                    <q-item-label caption>{{ dept.nama_site }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="availableDepts.length === 0">
                  <q-item-section class="tw-text-center tw-text-slate-500">
                    No available departments
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
            
            <div class="col-6">
              <div class="tw-font-semibold tw-text-slate-700 tw-mb-2">Selected Departments</div>
              <q-list bordered class="tw-rounded-lg tw-max-h-[400px] tw-overflow-auto">
                <q-item 
                  v-for="(dept, index) in selectedDepts" 
                  :key="dept.id_dept"
                >
                  <q-item-section>
                    <q-item-label>{{ dept.nama }}</q-item-label>
                    <q-item-label caption>{{ dept.nama_site }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-btn
                      round
                      dense
                      flat
                      color="red-6"
                      icon="remove_circle"
                      @click="removeDepartment(index)"
                    />
                  </q-item-section>
                </q-item>
                <q-item v-if="selectedDepts.length === 0">
                  <q-item-section class="tw-text-center tw-text-slate-500">
                    No departments selected
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
          </div>
        </q-card-section>
        <q-separator class="tw-bg-slate-200"/>
        <q-card-actions align="right" class="tw-p-6 tw-bg-slate-50">
          <q-btn
            label="Cancel"
            color="red-7"
            push
            icon="close"
            class="tw-px-6 tw-font-semibold"
            v-close-popup
          />
          <q-btn
            label="Save"
            :color="`${domain()}`"
            push
            icon="save"
            class="tw-px-6 tw-font-semibold"
            @click="saveData"
          />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
</template>

<script setup>
import { ref, onMounted, reactive } from "vue";
import axios from "axios";
import { domain, empid, spinnerBall } from "./../../utils";
import { useQuasar } from "quasar";
import { useNotify } from "./../../composables/useNotify";
import "./../../assets/styles/table.css";

const $q = useQuasar();
const { success, error } = useNotify();

const columns = [
  {
    name: "aksi",
    required: true,
    label: "Action",
    align: "left",
    field: "aksi",
    classes: 'sticky-column-left',
    headerClasses: 'sticky-column-left-header'
  },
  {
    name: "nama",
    align: "left",
    label: "Group Name",
    field: "nama",
    sortable: true,
  },
  {
    name: "dept",
    align: "left",
    label: "Departments",
    field: "dept",
    sortable: false,
  },
];

const listGroupDepartments = ref([]);
const availableDepts = ref([]);
const selectedDepts = ref([]);
const loading = ref(false);
const dialogForm = ref(false);
const isEdit = ref(false);

const pagination = ref({
  sortBy: "nama",
  descending: false,
  page: 1,
  rowsPerPage: 10,
  rowsNumber: 0,
  filter: null,
});

const form = reactive({
  id_group: null,
  nama: null,
});

const getData = async () => {
  try {
    loading.value = true;
    
    const res = await axios.get(`${import.meta.env.VITE_API}listGroupDepartments`, {
      params: pagination.value,
    });
    
    if (typeof res.data.data === "undefined") {
      listGroupDepartments.value = res.data;
    } else {
      listGroupDepartments.value = res.data.data;
      pagination.value.rowsNumber = res.data.pagination.total;
    }
    
    loading.value = false;
  } catch (err) {
    loading.value = false;
    error(err?.response?.data?.message || 'Failed to load data');
  }
};

const loadAvailableDepts = async () => {
  try {
    const params = form.id_group ? { grp_id: form.id_group } : {};
    const res = await axios.get(`${import.meta.env.VITE_API}getAvailableDepartments`, { params });
    availableDepts.value = res.data.filter(dept => 
      !selectedDepts.value.find(sd => sd.id_dept === dept.id_dept)
    );
  } catch (err) {
    error('Failed to load departments');
  }
};

const addDepartment = (dept) => {
  selectedDepts.value.push(dept);
  loadAvailableDepts();
};

const removeDepartment = (index) => {
  selectedDepts.value.splice(index, 1);
  loadAvailableDepts();
};

const openAddDialog = async () => {
  isEdit.value = false;
  resetForm();
  selectedDepts.value = [];
  await loadAvailableDepts();
  dialogForm.value = true;
};

const openEditDialog = async (row) => {
  isEdit.value = true;
  form.id_group = row.id_group;
  form.nama = row.nama;
  
  // Load selected departments
  try {
    const res = await axios.get(`${import.meta.env.VITE_API}getGroupDepartments`, {
      params: { grp_id: row.id_group }
    });
    selectedDepts.value = res.data;
  } catch (err) {
    error('Failed to load group departments');
  }
  
  await loadAvailableDepts();
  dialogForm.value = true;
};

const saveData = async () => {
  if (!form.nama) {
    error('Group name is required');
    return;
  }
  
  if (selectedDepts.value.length === 0) {
    error('At least one department must be selected');
    return;
  }
  
  try {
    spinnerBall();
    
    const deptIds = selectedDepts.value.map(d => d.id_dept);
    
    const payload = {
      id_group: form.id_group,
      nama: form.nama,
      id_dept: deptIds,
      creator: empid(),
    };
    
    await axios.post(`${import.meta.env.VITE_API}saveGroupDepartment`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'Group updated successfully' : 'Group added successfully');
    onRequest({
      pagination: pagination.value,
    });
    $q.loading.hide();
  } catch (err) {
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save group');
  }
};

const deleteData = async (row) => {
  $q.dialog({
    title: "Confirm Delete",
    message: `Are you sure you want to delete group <strong>${row.nama}</strong>?`,
    html: true,
    class: `side-${domain()} tw-rounded-2xl`,
    ok: {
      push: true,
      color: "red-7",
      label: "Delete",
      icon: "delete",
      class: "tw-font-semibold tw-px-6 tw-rounded-lg"
    },
    cancel: {
      push: true,
      color: 'grey-7',
      label: "Cancel",
      icon: "cancel",
      class: "tw-font-semibold tw-px-6 tw-rounded-lg"
    },
    persistent: true,
  }).onOk(async () => {
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deleteGroupDepartment`, {
        id_group: row.id_group_encrypted,
        creator: empid(),
      });
      success('Group deleted successfully');
      onRequest({
        pagination: pagination.value,
      });
      $q.loading.hide();
    } catch (err) {
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete group');
    }
  });
};

const resetForm = () => {
  form.id_group = null;
  form.nama = null;
};

const onRequest = (props) => {
  const { page, rowsPerPage, sortBy, descending, filter } = props.pagination;
  pagination.value.filter = filter;
  pagination.value.page = page;
  pagination.value.rowsPerPage = rowsPerPage;
  pagination.value.sortBy = sortBy;
  pagination.value.descending = descending;
  getData();
};

const updateTable = async () => {
  onRequest({
    pagination: pagination.value,
  });
};

onMounted(() => {
  onRequest({
    pagination: pagination.value,
  });
});
</script>
