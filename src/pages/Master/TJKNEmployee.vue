    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="person" size="28px" class="tw-text-white" />
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Master TJKN Employee</div>
            <div class="tw-flex tw-items-center tw-gap-2 tw-text-blue-100 tw-text-xs">
              <q-icon name="home" size="14px"/>
              <q-icon name="chevron_right" size="14px"/>
              <span>Master</span>
              <q-icon name="chevron_right" size="14px"/>
              <span>TJKN Employee</span>
            </div>
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section class="tw-bg-white">
        ...
        <div class="tw-mb-4">
          <q-input
            ...
            outlined
            dense
            label="Filter by Employee NIK"
            placeholder="Enter NIK or leave empty for all"
            class="tw-max-w-md"
            @update:model-value="onEmployeeFilterChange"
          >
          ...
          <div v-if="employeeFilter" class="tw-mt-2 tw-text-sm tw-text-orange-600 tw-font-semibold">
            Showing TJKN records for NIK: {{ employeeFilter }}
          </div>
        </div>
        
        <q-table
          :rows="listTJKNEmployee"
          :columns="columns"
          row-key="tjkn_id"
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12">
              <q-input
                ...
                outlined
                label="Pengurang TJKN"
                type="number"
                :rules="[val => !!val || 'Pengurang TJKN is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
...
const getData = async () => {
  try {
    ...
    const params = {
      ...pagination.value,
      nik: employeeFilter.value || 'all'
    };
    
    const res = await axios.get(`${import.meta.env.VITE_API}listTJKNEmployee`, {
      params,
    });
    
    if (typeof res.data.data === "undefined") {
      listTJKNEmployee.value = res.data;
    } else {
      listTJKNEmployee.value = res.data.data;
...
const openAddDialog = async () => {
  ...
  if (employeeFilter.value) {
    form.tjkn_nik = employeeFilter.value;
  }
...
const saveData = async () => {
  if (!form.tjkn_nik || !form.tjkn_year || !form.tjkn_month || !form.tjkn_duration) {
  ...
  try {
    ...
    await axios.post(`${import.meta.env.VITE_API}saveTJKNEmployee`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'TJKN Employee updated successfully' : 'TJKN Employee added successfully');
    onRequest({
    ...
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save TJKN Employee');
  }
};
...
const deleteData = async (row) => {
  $q.dialog({
    ...
    message: `Are you sure you want to delete TJKN Employee for <strong>${row.tjkn_nik} - ${row.tjkn_year} - ${row.month_name}</strong>?`,
    html: true,
    ...
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deleteTJKNEmployee`, {
        tjkn_id: row.tjkn_id_encrypted,
        creator: empid(),
      });
      success('TJKN Employee deleted successfully');
      onRequest({
        pagination: pagination.value,
      });
      $q.loading.hide();
      ...
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete TJKN Employee');
    }
  });
};
...
onMounted(() => {
  // Check if NIK is passed from Employee page
  if (route.query.nik) {
    employeeFilter.value = route.query.nik;
  }
