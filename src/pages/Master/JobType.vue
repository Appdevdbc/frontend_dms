    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="work" size="28px" class="tw-text-white" />
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Job Type Master</div>
      ...
      <q-card class="tw-w-full tw-max-w-2xl tw-rounded-2xl tw-shadow-2xl">
        <q-card-section :class="`bg-${domain()}`">
          <div class="text-h5 tw-text-white tw-font-bold tw-flex tw-items-center tw-gap-3">
            <q-icon name="work" size="32px"/>
            {{ isEdit ? 'Edit' : 'Add' }} Job Type
          </div>
        </q-card-section>
        <q-separator/>
        <q-card-section class="tw-p-6">
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-input
                v-model="form.nama_job"
                outlined
                label="Job Type Name"
                :rules="[val => !!val || 'Job type name is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
...
const getData = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}listJobTypes`, {
      params: pagination.value,
    });
    
    if (typeof res.data.data === "undefined") {
      listJobTypes.value = res.data;
    } else {
      listJobTypes.value = res.data.data;
...
const openEditDialog = async (row) => {
  isEdit.value = true;
  form.id_job = row.id_job;
  form.nama_job = row.nama_job;
  dialogForm.value = true;
};
...
const saveData = async () => {
  ...
  try {
    ...
    const payload = {
      id_job: form.id_job,
      nama_job: form.nama_job,
      creator: empid(),
    };
    
    await axios.post(`${import.meta.env.VITE_API}saveJobType`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'Job type updated successfully' : 'Job type added successfully');
    onRequest({
    ...
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save job type');
  }
};
...
const deleteData = async (row) => {
  $q.dialog({
    ...
    message: `Are you sure you want to delete job type <strong>${row.nama_job}</strong>?`,
    html: true,
    ...
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deleteJobType`, {
        id_job: row.id_job_encrypted,
        creator: empid(),
      });
      ...
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete job type');
    }
  });
};
