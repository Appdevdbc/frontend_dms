    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`side-${domain()}-1 tw-py-6`">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="settings" size="28px" class="tw-text-white" />
          <div>
            <div class="text-h6 tw-text-white tw-font-bold">Machine Master</div>
          ...
          <div class="row q-col-gutter-md">
            <div class="col-12">
              <q-input
                ...
                outlined
                label="Machine Code"
                :rules="[val => !!val || 'Machine code is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
            ...
            <div class="col-12">
              <q-input
                ...
                outlined
                label="Machine Name"
                :rules="[val => !!val || 'Machine name is required']"
                class="tw-rounded-lg"
              >
                <template v-slot:prepend>
...
const getData = async () => {
  try {
    ...
    const res = await axios.get(`${import.meta.env.VITE_API}listMachines`, {
      params: pagination.value,
    });
    
    if (typeof res.data.data === "undefined") {
      listMachines.value = res.data;
    } else {
      listMachines.value = res.data.data;
...
const saveData = async () => {
  ...
  try {
    ...
    await axios.post(`${import.meta.env.VITE_API}saveMachine`, payload);
    
    dialogForm.value = false;
    success(isEdit.value ? 'Machine updated successfully' : 'Machine added successfully');
    onRequest({
    ...
    $q.loading.hide();
    error(err?.response?.data?.message || 'Failed to save machine');
  }
};
...
const deleteData = async (row) => {
  $q.dialog({
    ...
    message: `Are you sure you want to delete machine <strong>${row.nama}</strong>?`,
    html: true,
    ...
    try {
      spinnerBall();
      await axios.post(`${import.meta.env.VITE_API}deleteMachine`, {
        id: row.id_encrypted,
        creator: empid(),
      });
      ...
      $q.loading.hide();
      error(err?.response?.data?.message || 'Failed to delete machine');
    }
  });
};
