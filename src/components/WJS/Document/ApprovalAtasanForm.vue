    ...
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      <q-card-section :class="`bg-${domain()} tw-py-5`">
        <div class="text-h6 tw-text-white tw-font-bold tw-flex tw-items-center tw-gap-2">
          <q-icon name="assignment_turned_in" size="24px" />ADD CONTENT (APPROVAL)
        </div>
      </q-card-section>
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12 col-md-6">
              <div class="tw-grid tw-grid-cols-1 tw-gap-4">
                ...
                <q-input :model-value="docData.kondisi_doc_ket" outlined type="textarea" label="Keterangan Kondisi" rows="3" readonly class="tw-rounded-lg tw-bg-slate-50">
            ...
            <div class="col-12 col-md-6">
              <div class="tw-grid tw-grid-cols-1 tw-gap-4">
                ...
                <q-input :model-value="docData.account_name_arsiparis || docData.arsiparis_nama" outlined label="Arsiparis Lokasi" readonly class="tw-rounded-lg tw-bg-slate-50">
                  <template v-slot:prepend><q-icon name="person" color="blue-6" /></template>
                </q-input>

                <q-input :model-value="docData.account_email_arsiparis || docData.arsiparis_email" outlined label="Email Arsiparis" readonly class="tw-rounded-lg tw-bg-slate-50">
                  <template v-slot:prepend><q-icon name="email" color="blue-6" /></template>
                </q-input>

                <!-- Catatan approval -->
                <q-input v-model="catatan" outlined type="textarea" label="Catatan Approval" rows="3" class="tw-rounded-lg">
                  <template v-slot:prepend><q-icon name="notes" color="blue-6" /></template>
                </q-input>
          ...
          <q-card style="min-width:420px;max-width:520px">
            <q-card-section :class="`bg-${domain()} text-white`">
              <div class="text-h6 tw-flex tw-items-center tw-gap-2">
                <q-icon name="edit_note" />Revisi
              </div>
            </q-card-section>
            <q-card-section class="tw-pt-4">
              <q-input v-model="revisiReason" outlined type="textarea" label="Reason" rows="4"
                maxlength="200" counter class="tw-rounded-lg"
                hint="Maksimal 200 karakter" />
            </q-card-section>
            <q-separator />
            <q-card-actions align="right" class="tw-p-3">
              <q-btn flat label="Tutup" color="grey-6" v-close-popup />
              <q-btn unelevated label="Simpan" color="orange-6" icon="send"
                @click="submitRevisi" :loading="saving" />
            </q-card-actions>
          </q-card>
        </q-dialog>
