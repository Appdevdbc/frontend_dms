    ...
    <q-card class="tw-w-full tw-max-w-2xl tw-rounded-xl tw-shadow-2xl">
      <q-card-section class="tw-bg-gradient-to-r tw-from-orange-500 tw-to-orange-600 tw-text-white tw-py-4">
        <div class="tw-flex tw-items-center tw-justify-between">
          <div class="tw-flex tw-items-center tw-gap-3">
            ...
            <div>
              <div class="tw-text-xl tw-font-bold">Revisi Permintaan Dokumen</div>
      ...
      <q-card-section class="tw-p-6">
        <q-input v-model="reason" outlined type="textarea" rows="4" counter maxlength="500" class="tw-rounded-lg" label-slot>
          <template v-slot:prepend><q-icon name="report_problem" color="orange-6" /></template>
          <template v-slot:label><span class="tw-font-semibold tw-text-slate-700">Alasan Revisi</span><span class="tw-text-red-500 tw-font-bold">*</span></template>
        </q-input>
      </q-card-section>
      <q-card-actions class="tw-p-6 tw-pt-0 tw-flex tw-gap-3 tw-justify-end">
        ...
        <q-btn unelevated color="orange-6" label="Revisi" icon="edit_note" class="tw-font-semibold tw-px-6 tw-rounded-lg" @click="handleRevisi" :loading="loading" :disable="!reason.trim()" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
const handleRevisi = async () => {
  if (!reason.value?.trim()) { showError('Alasan revisi wajib diisi'); return; }
  try {
    loading.value = true;
    ...
    loading.value = false;
    success('Permintaan dikembalikan untuk revisi');
    emit('done');
    closeDialog();
  } catch (err) {
    loading.value = false;
